...
while len(products) > 0:
    ...
    if available_robots:
        ...
        current_robot['available_at_time'] = time + timedelta(seconds=robot['processing_time'])
        robot = [el for el in robots if el == current_robot[0]]
        robot['available_at_time'] = time + timedelta(seconds=robot['processing_time'])
        print(f"{robot['name']} - {current_product} [{time.strftime('%H:$M:$S')}]")
    else:
        for r in robots:
            if time >= r['available_at_Time']:
